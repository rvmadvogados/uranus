...
namespace Boleto2Net
{
    ...
    public class BoletoBancario : Control
    {
        ...
        public string GeraHtmlReciboSacado(string texto)
        {
            try
            {
                var html = new StringBuilder();
                ...
                {
                    html.Append($"<div class='w666' style='border-top: 1px dashed #000000; border-bottom: 1px dashed #000000; min-height: 301px; max-height: 301px;'>{texto}</div>");
                }
                else
                {
        ...
        private string MontaHtml(string urlImagemLogo, string urlImagemBarra, string imagemCodigoBarras)
        {
            ...
            if (!FormatoCarne)
            {
                ...
                if (!OcultarReciboSacado)
                {
                    html.Append(GeraHtmlReciboSacado(Boleto.TextHTML));
            ...
            return html
                ...
                .Replace("@CEDENTE_BOLETO", !Boleto.Banco.Cedente.MostrarCNPJnoBoleto ? Boleto.Banco.Cedente.Nome : string.Format("{0} - {1} - {2}", Boleto.Banco.Cedente.Nome, Utils.FormataCNPJ(Boleto.Banco.Cedente.CPFCNPJ), MontaEndereço(Boleto)))
                .Replace("@CEDENTE", string.Format("{0} - {1} - {2}", Boleto.Banco.Cedente.Nome, Utils.FormataCNPJ(Boleto.Banco.Cedente.CPFCNPJ), MontaEndereço(Boleto)))
        ...
        private string MontaEndereço(Boleto boleto)
        {
            var endereço = boleto.Banco.Cedente.Endereco.FormataLogradouro(40) + " - " 
                + boleto.Banco.Cedente.Endereco.Bairro + ", " 
                + boleto.Banco.Cedente.Endereco.Cidade + " - "
                + boleto.Banco.Cedente.Endereco.UF + " - CEP: " 
                + boleto.Banco.Cedente.Endereco.CEP;
        ...
        {//protected StringBuilder HtmlOffLine(string srcCorte, string srcLogo, string srcBarra, string srcPonto, string srcBarraInterna, string srcCodigoBarra)
            ...
            html.Append(MontaHtml(srcLogo, srcBarra, "<img src=\"" + srcCodigoBarra + "\" alt=\"Código de Barras\" />")); // width=\"450\" height=\"50\"
